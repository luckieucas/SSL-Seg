...
class SkipDenseNet3D(BaseModel):
    ...
    def test(self,device='cpu'):
        ...
        summary(self.to(torch.device(device)), (1, 64, 64, 64),device=device)
        print("SkipDenseNet3D test is complete")
if __name__ == "__main__":
    model = SkipDenseNet3D(in_channels=1, growth_rate=16, num_init_features=32, drop_rate=0.1,classes=4)
