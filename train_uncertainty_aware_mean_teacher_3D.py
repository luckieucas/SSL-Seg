...
parser.add_argument('--exp', type=str,
                    default='MMWHS_UAMT', help='experiment_name')
...
parser.add_argument('--consistency_rampup', type=float,
                    default=7500.0, help='consistency_rampup')
...
def train_UAMT(args, snapshot_path):
    exp = config['exp']
    ...
    def create_model(ema=False):
        # Network definition
        net = net_factory_3d(net_type=backbone,
    ...
    if dataset_name in ['BCV','MMWHS']:
        db_train = BCVDataset(img_list_file=train_list, patch_size=patch_size,
    ...
    for epoch_num in iterator:
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            loss_dice = dice_loss(outputs_soft[:labeled_bs], 
                                  label_batch[:labeled_bs].unsqueeze(1))
            supervised_loss = 0.5 * (loss_dice + loss_ce)
            consistency_weight = get_current_consistency_weight(iter_num//4)
            consistency_dist = losses.softmax_mse_loss(outputs[labeled_bs:], 
                                                       ema_output)  
                                                       # (batch, 2, 112,112,80)
            threshold = (0.75+
                         0.25*
                         ramps.sigmoid_rampup(iter_num,max_iterations))*np.log(2)
...
if __name__ == "__main__":
    ...
    train_UAMT(config, snapshot_path)
