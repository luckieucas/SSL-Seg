...
class TranspConvNet(nn.Module):
    ...
    def __init__(self, in_channels, classes):
        ...
        conv1 = conv1x1x1(transp_channels, conv_channels, stride=1)
...
class ResNetMed3D(BaseModel):
    ...
    def __init__(self, in_channels=3, classes=10,
        ...
        self.segm = TranspConvNet(in_channels=256 * block.expansion, classes=classes)
    ...
    def forward(self, x):
        ...
        x = self.layer2(x)
        x = self.layer3(x)
        bottom = self.layer4(x)
        cl_output = F.normalize(self.project1(bottom), 2, 1)
...
if __name__ == "__main__":
    model = generate_resnet3d(in_channels=1,model_depth=18)
    a = torch.rand(1, 1, 96, 160, 160)
    y = model(a)
    summary(model, (1, 96, 160, 160),device='cpu')
    print(y.shape)
