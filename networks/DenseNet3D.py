...
class SinglePathDenseNet(BaseModel):
    def __init__(self, in_channels, classes=4, drop_rate=0.1, return_logits=True, early_fusion=False):
        ...
        self.features.add_module('conv1x1_2', nn.Conv3d(256,
        ...
        self.features.add_module('conv1x1_3', nn.Conv3d(128,
        ...
        self.classifier.add_module('classifier', nn.Conv3d(64,
    ...
    def test(self,device='cpu'):
        ...
        input_tensor = torch.rand(1, self.input_channels, 64, 64, 64).to(device)
        ideal_out = torch.rand(1, self.num_classes, 64, 64, 64)
        out = self.forward(input_tensor)
        #assert ideal_out.shape == out.shape
        summary(self.to(torch.device(device)), (self.input_channels, 64, 64, 64),device=device)
...
class DualPathDenseNet(BaseModel):
    ...
    def test(self,device='cpu'):
        ...
        out = self.forward(input_tensor)
        assert ideal_out.shape == out.shape
        summary(self.to(torch.device(device)), (self.input_channels, 96, 160, 160),device=device)
        import torchsummaryX
        torchsummaryX.summary(self, input_tensor.to(device))
